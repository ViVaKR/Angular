import { Injectable, computed, PLATFORM_ID, Inject, Signal, inject } from '@angular/core';
...
export class ChatService {
  ...
  async connect(): Promise<void> {
    ...
    console.log('🔄 ChatService.connect() 호출');
  ...
  async disconnect(): Promise<void> {
    console.log('🔴 ChatService.disconnect() 호출');
  ...
  private setupReconnection(): void {
    this.conn.hub?.onclose((error) => {
      console.log('🔴 ChatService.onclose:', error);
  ...
  async GetToken(): Promise<void> {
    // 연결되지 않은 상태면 무시
    if (this.intentionalDisconnect || !this.isConnected) {
      console.log('⚠️ 연결되지 않은 상태 - 토큰 요청 무시');
      return;
    }
