...
import { catchError, EMPTY, finalize, from, Observable, of, retry, switchMap, tap, throwError, timer } from 'rxjs';
...
export class AuthService {
  ...
  signIn(data: Omit<ISignIn, 'deviceFingerprint'>): Observable<IAuthResponse> {
    return from(this.fingerprinterService.generateFingerprint()).pipe(
      switchMap(fp => {
        const payload: ISignIn = { ...data, deviceFingerprint: fp, };
        return this.http.post<IAuthResponse>(`${this.baseUrl}/account/signin`, payload);
      }), tap(async (res) => {
        // 2. 2FA 필요
