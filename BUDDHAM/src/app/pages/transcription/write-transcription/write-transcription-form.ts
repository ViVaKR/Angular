import { inject, Injectable } from '@angular/core';
import { FormGroup } from '@angular/forms';
import { FormService } from '@app/core/services/form-service';
import { TranscriptionService } from '@app/core/services/transcription-service';
import { SCRIPTURE_CONTENT } from '@app/forms/form-configs';

@Injectable({
  providedIn: 'root',
})
export class WriteTranscriptionForm {
  private formService = inject(FormService);

  private trascriptionService = inject(TranscriptionService);

  public fields = SCRIPTURE_CONTENT;

  readonly form: FormGroup = this.formService.createForm(this.fields, this.trascriptionService);

  submitValue() {

    if (this.form.invalid) {
      this.form.markAllAsTouched();
      return null;
    }
    return this.form.getRawValue();
  }

  hasError(fieldName: string, errorType: string): boolean {
    return this.formService.hasError(this.form, fieldName, errorType);
  }

  isFieldPending(fieldName: string): boolean {
    return this.formService.isFieldPending(this.form, fieldName);
  }

  reset(value?: any) {
    this.formService.deepResetForm(this.form, value);
  }
}
